...
function App() {
  ...
  useEffect(() => {
    // Tie this reset to the metadata change.
    ...
    if ("track" in metadata.metadata) {
      ...
      return (
        <TrackInfo
          ...
          availability={availability.availabilityMap.get(track.spotify_id || "")}
          downloadingCover={cover.downloadingCoverTrack === (track.spotify_id || `${track.name}-${track.artists}`)}
          downloadedCover={cover.downloadedCovers.has(track.spotify_id || `${track.name}-${track.artists}`)}
          failedCover={cover.failedCovers.has(track.spotify_id || `${track.name}-${track.artists}`)}
          skippedCover={cover.skippedCovers.has(track.spotify_id || `${track.name}-${track.artists}`)}
    ...
    if ("album_info" in metadata.metadata) {
      ...
      return (
        <AlbumInfo
          ...
          onDownloadAll={() => download.handleDownloadAll(track_list, album_info.name, true)}
          onDownloadSelected={() => download.handleDownloadSelected(selectedTracks, track_list, album_info.name, true)}
    ...
    if ("playlist_info" in metadata.metadata) {
      ...
      return (
        <PlaylistInfo
          ...
          onDownloadAll={() => download.handleDownloadAll(track_list, playlist_info.name)}
          onDownloadSelected={() => download.handleDownloadSelected(selectedTracks, track_list, playlist_info.name)}
          onStopDownload={download.handleStopDownload}
          onOpenFolder={handleOpenFolder}
          onPageChange={setCurrentListPage}
    ...
    if ("artist_info" in metadata.metadata) {
      ...
      return (
        <ArtistInfo
          ...
          onDownloadSelected={() => download.handleDownloadSelected(selectedTracks, track_list, artist_info.name)}
          onStopDownload={download.handleStopDownload}
          onOpenFolder={handleOpenFolder}
          onAlbumClick={metadata.handleAlbumClick}
          onArtistClick={async (artist) => {
          ...
          <>
            ...
            <Dialog open={metadata.showAlbumDialog} onOpenChange={metadata.setShowAlbumDialog}>
              <DialogContent className="sm:max-w-[425px] p-6 [&>button]:hidden">
                ...
                <DialogDescription>Do you want to fetch metadata for this album?</DialogDescription>
        ...
        <Dialog open={isFFmpegInstalled === false} onOpenChange={() => {}}>
          <DialogContent className="max-w-[360px] [&>button]:hidden p-6 gap-5">
            <DialogHeader className="space-y-2">
              <DialogTitle className="text-lg font-bold tracking-tight">FFmpeg Required</DialogTitle>
              <DialogDescription className="text-sm text-foreground/70 leading-relaxed font-normal">
                FFmpeg is essential for SpotiFLAC to function properly. This setup will download about{" "}
                <span className="text-foreground font-semibold">100-200MB</span> of data.
              </DialogDescription>
            </DialogHeader>
              ...
              <div className="space-y-4">
                {ffmpegInstallStatus === "extracting" ? (
                  <div className="flex flex-col items-center justify-center py-2 animate-in fade-in duration-500">
                    ...
                    <span className="text-[10px] text-muted-foreground uppercase tracking-[0.2em] font-bold mt-2">
                      Finalizing setup
                    </span>
                  </div>
                ) : (
                  <div className="space-y-3">
                    <div className="flex justify-between text-[11px] font-bold">
                      <div className="flex flex-col gap-0.5">
                        ...
                        {downloadProgress.is_downloading && downloadProgress.mb_downloaded > 0 && (
                    ...
                    <div className="h-1.5 w-full bg-secondary/30 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-primary transition-all duration-300 shadow-[0_0_10px_rgba(var(--primary),0.3)]"
            ...
            <DialogFooter className="flex-row gap-3 pt-2">
              ...
              <Button
                className={`${isInstallingFFmpeg ? "w-full" : "flex-1"} h-11 text-sm font-bold shadow-lg shadow-primary/10`}
