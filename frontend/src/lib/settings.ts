...
export interface Settings {
  ...
  // legacy fields (still used in a couple places)
...
export const FILENAME_PRESETS: Record<
  FilenamePreset,
  { label: string; template: string }
> = {
...
export const DEFAULT_SETTINGS: Settings = {
  ...
  // Favor quality defaults used by the older branch, but keep auto/amazon settings.
...
function migrateSettingsShape(parsed: any): Settings {
  ...
  if (
    !("folderPreset" in parsed) &&
    ("artistSubfolder" in parsed || "albumSubfolder" in parsed)
  ) {
  ...
  if (!("filenamePreset" in parsed) && "filenameFormat" in parsed) {
    ...
    // Keep consistent mapping to templates.
  ...
  if (!("tidalQuality" in parsed)) parsed.tidalQuality = DEFAULT_SETTINGS.tidalQuality;
  if (!("qobuzQuality" in parsed)) parsed.qobuzQuality = DEFAULT_SETTINGS.qobuzQuality;

  // Backward compat: some builds used a different numeric encoding.
  if (parsed.qobuzQuality === "27") parsed.qobuzQuality = "6";
...
function getSettingsFromLocalStorage(): Settings {
  try {
    ...
    if (stored) {
      ...
      return migrateSettingsShape(parsed);
    }
  } catch (error) {
...
export function getSettings(): Settings {
  return cachedSettings || getSettingsFromLocalStorage();
}

export async function loadSettings(): Promise<Settings> {
  // This repo's checked-in Wails bindings are minimal and do not expose Load/Save.
  // Keep settings working using LocalStorage.
...
export function parseTemplate(template: string, data: TemplateData): string {
  ...
  result = result.replace(
    /\{album_artist\}/g,
    data.album_artist || data.artist || "Unknown Artist"
  );
  result = result.replace(
    /\{track\}/g,
    data.track ? String(data.track).padStart(2, "0") : "00"
  );
